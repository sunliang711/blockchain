...
start(){
    ...
    tmux new-session -s eth-node  "bash 02-start.sh $configFile || bash"\; split-window -v "bash 03-logfile.sh $configFile || bash"\; split-window -h "bash 04-console.sh $configFile || bash"\; 
}
